...
const buildStep2Body = (data: Step2FormData) => ({
  identityType: toIdentityType(data.identityType),
...
export const applicationApi = baseApi.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    getMyApplications: builder.query<LoanApplication[], void>({
      query: () => "/applications",
      transformResponse: (response: { data: LoanApplication[] } | LoanApplication[]) =>
        Array.isArray(response) ? response : (response as { data: LoanApplication[] }).data ?? [],
    ...
    saveStep1: builder.mutation<LoanApplication, { id: string; data: Step1FormData }>({
      query: ({ id, data }) => ({
    ...
    saveStep2: builder.mutation<LoanApplication, { id: string; data: Step2FormData }>({
      query: ({ id, data }) => ({
    ...
    saveStep3: builder.mutation<LoanApplication, { id: string; data: Step3FormData }>({
      query: ({ id, data }) => ({
