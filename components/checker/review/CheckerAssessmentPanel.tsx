...
import { CheckerApprovalActions } from "./CheckerApprovalActions";
...
/**
 * Left column of the Check review layout — every prior stage's information,
 * read-only, followed by the Checker's own real tick (Check) action.
 ...
 */
export function CheckerAssessmentPanel({ detail }: { detail: CheckerApplicationDetail }) {
  ...
  return (
    <div>
      <div className="md:sticky md:top-0 z-10 px-5 lg:px-6 py-3 border-b border-border bg-card flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold text-foreground">Check Review</h2>
      </div>
        ...
        <div className="p-5 lg:p-6 space-y-5">
          ...
          <section className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Displayed exactly as submitted by the Initiator. Every field below is read-only — Checker cannot modify any value.
            </p>
            <AssessmentSummary />
          </section>
          ...
          <section className="space-y-3">
            <h3 className="text-xs font-semibold text-foreground uppercase tracking-wide">Check Decision</h3>
            <CheckerApprovalActions applicationId={detail.id} stage={detail.stage} />
          </section>
        </div>
      </Form>
    </div>
