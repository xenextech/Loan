...
import { CheckerAssessmentPanel } from "./CheckerAssessmentPanel";
import type { CheckerApplicationDetail } from "../types/checker";

/**
 * Check review layout. The verification form opens full-width; the
 ...
 * the Supporter's `SupporterVerificationLayout`, and the Approver's
 * `ApproverVerificationLayout`.
 */
export function CheckerVerificationLayout({ detail }: { detail: CheckerApplicationDetail }) {
  ...
  return (
    <div className="flex flex-col">
      <header className="sticky top-0 z-30 bg-card border-b border-border px-4 sm:px-6 py-3 lg:h-16 lg:py-0 flex items-center gap-3">
        <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground shrink-0" onClick={() => router.push("/checker")}>
      ...
      <div className="flex flex-col md:flex-row md:items-start">
        ...
        >
          <CheckerAssessmentPanel detail={detail} />
        </div>
        <div
          className={cn(
