...
export default function EnrollmentCertificateGenerator() {
  ...
  const [qrVerifyUrl, setQrVerifyUrl] = useState(() => `verify.genzloan.com.np/doc/${makeDefaultToken()}`);

  const [generated, setGenerated] = useState(false);
  const [savedId, setSavedId] = useState<string | null>(null);
  const [createEnrollmentCert, { isLoading: isSaving }] = useCreateEnrollmentCertMutation();
  ...
  const handleSave = async () => {
    ...
    toast.success("Enrollment certificate saved", { description: `ID: ${response.data.id.slice(0, 8)}…` });
  };
  ...
  const handleDownloadPDF = () => {
    ...
    const certRows = [
      ["Active Enrollment",      isEnrolled       ? "✓ Confirmed — Currently Enrolled"    : "✗ Not Confirmed"],
      ["Academic Backlogs",      hasBacklogs      ? "✗ Yes (has backlogs)"                : "✓ None"],
      ["Disciplinary Hold",      disciplinaryHold ? "✗ Yes — Hold Placed"                 : "✓ None"],
...
<body>
  ...
  <div class="cert-box">
    ...
    the institution and is digitally registered on the GenZ Loan platform.<br>
  ...
  <div class="footer">
    Generated via GenZ Loan Digital Platform · NRB Unified Directive 2081 · ${collegeName}${collegeRegNo ? ` · Reg. ${collegeRegNo}` : ""}
  </div>
  ...
  return (
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Enrollment Certificate</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Certify student enrollment and academic standing for education loan processing.
        </p>
      </motion.div>
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-5 py-4 border-b border-border">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-sm font-semibold text-foreground">Enrollment Certificate</CardTitle>
            ...
            <CardContent className="p-5 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Field label="College code (for ref no.)">
                    <Input className="h-8 text-xs" placeholder="e.g. AIM, KMC, PU" value={collegeCode} onChange={e => setCollegeCode(e.target.value)} />
                  </Field>
                  <Field label="Address">
                  ...
                  <Field label="Affiliation">
                    <Input className="h-8 text-xs" placeholder="e.g. Tribhuvan University" value={collegeAffiliation} onChange={e => setCollegeAffiliation(e.target.value)} />
                  </Field>
                  <Field label="Phone">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <Field label={`Reference no. ${collegeCode ? `(${collegeCode.toUpperCase()}/${refNo})` : ""}`}>
                    <Input
                ...
                {collegeCode && (
                  <p className="text-[10px] text-muted-foreground mt-2">
                    Full ref: <span className="font-mono text-foreground">{fullRefNo}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Field label="Program / Course">
                    <Input className="h-8 text-xs" placeholder="e.g. BBA, BIT, B.Sc. CSIT" value={programName} onChange={e => setProgramName(e.target.value)} />
                  </Field>
              ...
              <div>
                ...
                <div className="rounded-lg border border-border overflow-hidden p-2">
                  <CertToggle label="Student is actively enrolled" checked={isEnrolled} onChange={setIsEnrolled} positiveLabel="Yes" negativeLabel="No" />
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-4 py-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] text-muted-foreground">Enrollment Certificate</span>
              </div>
            </CardHeader>
            <CardContent className="p-3">
              <div className="bg-white rounded-lg p-4 font-mono text-[9.5px] border border-border shadow-sm leading-relaxed">
                ...
                <div className="flex justify-between text-[8px] text-muted-foreground mb-2">
                  <span><strong>Ref.:</strong> {fullRefNo}</span>
                ...
                <div className="border-2 border-primary text-center py-2 mb-3 rounded">
                  <p className="text-[10px] font-extrabold text-primary tracking-widest uppercase">Enrollment Certificate</p>
                ...
                <p className="text-[8.5px] text-muted-foreground leading-snug mb-3">
                  ...
                  {tuRollNo && <>, bearing Roll No. <strong className="text-foreground">{tuRollNo}</strong>,</>}
                ...
                <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">Academic Status</p>
                ...
                <table className="w-full text-[8px] border-collapse mb-2">
                  <tbody>
                    {[
                      { label: "Active Enrollment",     icon: certIcon(isEnrolled,       true),  text: isEnrolled ? "Confirmed" : "Not Confirmed" },
                      { label: "Academic Backlogs",      icon: certIcon(hasBacklogs,      false), text: hasBacklogs ? "Yes (has backlogs)" : "None" },
                      { label: "Disciplinary Hold",      icon: certIcon(disciplinaryHold, false), text: disciplinaryHold ? "Yes — Hold Placed" : "None" },
                      { label: "Fee Dues (NPR)",         icon: certIcon(feeDue === 0,     true),  text: feeDue > 0 ? `Rs ${feeDue.toLocaleString("en-IN")}` : "Nil (Fully Paid)" },
                ...
                <div className="bg-[oklch(0.62_0.18_145)]/8 border-l-2 border-[oklch(0.62_0.18_145)] px-2 py-1.5 mb-3 rounded-r text-[8px] leading-snug">
                  <strong>Official cert by:</strong> {collegeName || "[College]"}<br />
                  <span className="text-primary">Verify: {qrVerifyUrl} · Token: {qrToken}</span>
                </div>
