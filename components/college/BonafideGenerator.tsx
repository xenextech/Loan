...
export default function AgreementGenerator() {
  ...
  const [refNo, setRefNo] = useState(
    () =>
      `Agreement-${new Date().getFullYear()}-${String(Date.now()).slice(-4)}`,
  );
            ...
            <CardContent className="p-1.5 space-y-0.5">
              {SECTIONS.map((s) => (
                ...
                >
                  ...
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${sectionFilled[s.id] ? "bg-[oklch(0.62_0.18_145)]" : "bg-muted-foreground/25"}`}
                  />
                </button>
              ))}
            </CardContent>
            ...
            <CardContent className="p-3">
              <div className="bg-white rounded-lg p-4 font-mono text-[9.5px] border border-border shadow-sm leading-relaxed">
                    ...
                    {[
                      {
                        ...
                        val: certIcon(isEnrolled, true),
                        text: isEnrolled ? "Yes" : "No",
                      },
                      {
                        label: "Academic Backlogs",
                        val: certIcon(hasBacklogs, false),
                        text: hasBacklogs ? "Yes (has backlogs)" : "None",
                      },
                      {
                        label: "Disciplinary Hold",
                        val: certIcon(disciplinaryHold, false),
                        text: disciplinaryHold ? "Yes" : "None",
                      },
                      {
                        label: "Fee Dues (NPR)",
                        val: certIcon(feeDue === 0, true),
                        text:
                          feeDue > 0
                ...
                <div className="bg-[oklch(0.62_0.18_145)]/8 border-l-2 border-[oklch(0.62_0.18_145)] px-2 py-1.5 mb-3 rounded-r text-[8px] leading-snug">
                  <strong>Verified by:</strong> {collegeName || "[College]"}
                  <br />
