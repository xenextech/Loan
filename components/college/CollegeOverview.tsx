...
const STATS = [
  ...
  },
  {
    ...
    positive: true,
    iconBg:
      "bg-[oklch(0.528_0.113_235.573)]/15 text-[oklch(0.528_0.113_235.573)]",
  },
  {
    icon: Clock,
...
const PARTNER_BANKS = [
  ...
  {
    name: "Laxmi Sunrise",
    detail: "10.0% · 14 slots",
    status: "active" as const,
  },
  { name: "Kumari Bank", detail: "Renew MOU 2081", status: "renew" as const },
  { name: "Unnati", detail: "Digital · 9.25%", status: "digital" as const },
...
function verificationStageBadge(v: CollegeMyVerification): {
  label: string;
  style: string;
} {
  ...
  if (v.isApplicationVerified) {
    return {
      label: "Verified",
      style:
        "bg-[oklch(0.62_0.18_145)]/15 text-[oklch(0.42_0.18_145)] border-0",
    };
  }
  return {
    label: "Pending verify",
    style: "bg-[oklch(0.75_0.18_80)]/15 text-[oklch(0.55_0.18_80)] border-0",
  };
}
    ...
    >
      <Card className="border-border shadow-none">
        <CardContent className="px-5 py-5">
          <div
            className={`w-9 h-9 rounded-xl ${stat.iconBg} flex items-center justify-center mb-4`}
          >
          ...
          <p
            className={`text-[11px] mt-1 ${stat.positive ? "text-[oklch(0.42_0.18_145)]" : "text-destructive"}`}
          >
            {stat.sub}
          </p>
        </CardContent>
...
const QUICK_ACTIONS = [
  {
    label: "Generate admission offer letter",
    href: "/college/offer-letter",
    primary: true,
  },
  {
    label: "Create bank / Unnati agreement",
    href: "/college/agreements",
    primary: false,
  },
  {
    label: "Add student to directory",
    href: "/college/students",
    primary: false,
  },
  {
    label: "Calculate fees and loan amount",
...
export default function CollegeOverview() {
  ...
  const verifiedCount =
    verifications?.filter((v) => v.isApplicationVerified).length ?? 0;
  const offerLetterCount =
    verifications?.filter((v) => v.offerLetterUploaded).length ?? 0;
  const pendingCount =
    verifications?.filter(
      (v) => !v.isApplicationVerified && v.applicationStatus === "SUBMITTED",
    ).length ?? 0;

  const liveStats = [
    {
      ...STATS[0],
      value: verLoading ? "—" : String(offerLetterCount),
      sub: `of ${totalApps} total`,
    },
    {
      ...STATS[1],
      value: verLoading ? "—" : String(totalApps),
      sub: `${verifiedCount} verified`,
    },
    {
      ...STATS[2],
      value: verLoading ? "—" : String(verifiedCount),
      sub: "enrollment confirmed",
    },
    {
      ...STATS[3],
      value: verLoading ? "—" : String(pendingCount),
              ...
              <CardContent className="p-3 space-y-1.5">
                {QUICK_ACTIONS.map((a) => (
                  <Link key={a.label} href={a.href}>
                    <button
                      type="button"
                      className={`w-full text-left flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-xs font-medium transition-colors ${
                        a.primary
              ...
              <CardContent className="px-5 py-3 space-y-2">
                {PARTNER_BANKS.map((b) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-[11px] font-medium ${bankDetailStyles[b.status]}`}
                    >
                      {b.detail}
                    </span>
                  </div>
              ...
              {verLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-14 px-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground max-w-xs">
                    Applications you verify via college verification links will
                    appear here.
                  </p>
                </div>
              ) : (
                        ...
                        >
                          ...
                          <TableCell className="py-3.5">
                            <span className="text-xs font-semibold text-foreground">
                              {v.loanAmount != null
                                ? formatNPRShort(v.loanAmount)
                                : "—"}
                            </span>
                          </TableCell>
                          <TableCell className="py-3.5">
                            <Badge
                              className={`text-[10px] font-semibold ${badge.style}`}
                            >
                              {badge.label}
                            </Badge>
                          </TableCell>
