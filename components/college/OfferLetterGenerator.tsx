...
const DEFAULT_SIGNATORIES = [
  { name: "", designation: "Head of Admissions",       stamp_area_label: "Admission Office Seal" },
  { name: "", designation: "Principal / Campus Chief", stamp_area_label: "Principal's Seal" },
  { name: "", designation: "Finance Officer",          stamp_area_label: "Finance Seal" },
];
...
export default function OfferLetterGenerator() {
  ...
  const [qrVerifyUrl, setQrVerifyUrl] = useState(() => `verify.genzloan.com.np/doc/${qrToken}`);
  ...
  const autoTotal   = af + (tf + ef + lf) * sems;
  const displayTotal = totalManual && totalApprox !== "" ? Number(totalApprox) : autoTotal;
  ...
  const [createOfferLetter, { isLoading: isSaving }] = useCreateOfferLetterMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await createOfferLetter({
        ...
        document: { refNo, issuedDateAD: issuedDateAD || undefined, issuedDateBS: issuedDateBS || undefined, validUntilAD: validUntilAD || undefined, validUntilBS: validUntilBS || undefined },
        student: { studentFullName: studentName, studentDobAD: studentDobAD || undefined, studentDobBS: studentDobBS || undefined, citizenshipNo: citizenshipNo || undefined, fatherName: fatherName || undefined, motherName: motherName || undefined, permanentAddress: permanentAddress || undefined, district: district || undefined, province: province || undefined },
        program: { programName: progName || undefined, programFullName: progFullName || undefined, programAffiliation: progAffiliation || undefined, durationYears: durationYears ? Number(durationYears) : undefined, totalSemesters: totalSemesters ? Number(totalSemesters) : undefined, creditHours: creditHours ? Number(creditHours) : undefined, academicYearBS: academicYearBS || undefined, intakeMonthBS: intakeMonthBS || undefined },
        fees: { admissionFee: admissionFee !== "" ? admissionFee : undefined, tuitionPerSem: tuitionPerSem !== "" ? tuitionPerSem : undefined, examFeePerSem: examFeePerSem !== "" ? examFeePerSem : undefined, labFeePerSem: labFeePerSem !== "" ? labFeePerSem : undefined, totalApprox: displayTotal || undefined },
        conditions: conditions.filter(Boolean),
        signatories: signatories.map(s => ({ name: s.name, designation: s.designation, stampAreaLabel: s.stamp_area_label || undefined })),
        qr: { qrToken: qrToken || undefined, qrVerifyUrl: qrVerifyUrl || undefined },
      }).unwrap();
      toast.success("Offer letter saved", { description: `ID: ${response.id.slice(0, 8)}…` });
      resetForm();
    } catch {
  ...
  const handleDownloadPDF = () => {
    ...
    const conditionRows = conditions
      .filter(Boolean)
      .map((c, i) => `<li style="margin-bottom:5px;font-size:9.5pt">${c}</li>`)
      .join("");

    const sigCols = signatories
      .map(s => `
        <div style="text-align:center;width:${Math.floor(100 / signatories.length)}%">
          ...
          <div style="font-size:8pt;color:#9ca3af;font-style:italic">[${s.stamp_area_label}]</div>
        </div>`)
      .join("");

    const programRows = [
      ["Program (Short)", progName || "—"],
      ["Program (Full)",  progFullName || "—"],
      ["Affiliation",     progAffiliation || collegeAffiliation || "—"],
      ["Duration",        durationYears ? `${durationYears} Year${Number(durationYears) !== 1 ? "s" : ""}` : "—"],
      ["Total Semesters", totalSemesters || "—"],
      ["Credit Hours",    creditHours || "—"],
      ["Academic Year (BS)", academicYearBS || "—"],
      ["Intake Month (BS)",  intakeMonthBS || "—"],
    ].map(([f, v], i) => `<tr style="background:${i%2===1?"#f9fafb":"#fff"}"><td style="padding:6px 10px;border:1px solid #d1d5db;width:42%;color:#6b7280">${f}</td><td style="padding:6px 10px;border:1px solid #d1d5db;font-weight:500">${v}</td></tr>`).join("");
    ...
    const feeRows = [
      ...
      ["Total (Approx)",           displayTotal > 0 ? fmtNPR(displayTotal) + "/- (NPR)" : "—"],
    ].map(([f, v], i) => `<tr style="background:${i%2===1?"#f9fafb":"#fff"}"><td style="padding:6px 10px;border:1px solid #d1d5db;width:42%;color:#6b7280">${f}</td><td style="padding:6px 10px;border:1px solid #d1d5db;${f==="Total (Approx)"?"font-weight:700;color:#166534":""}">${v}</td></tr>`).join("");
...
<body>
  ...
  <div class="title-banner">
    ...
    <p>${progFullName || progName || "Program"} · ${progAffiliation || collegeAffiliation || "Affiliated University"}</p>
  </div>
  ...
  <div class="body-text">
    ...
    ${academicYearBS}. This offer is issued as per ${progAffiliation || collegeAffiliation || "University"} guidelines
  ...
  const handleGenerate = () => {
    ...
    setGenerated(true);
    toast.success("Offer letter ready", { description: "Click Download PDF to save." });
  };
  ...
  function updateSignatory(idx: number, field: keyof typeof signatories[0], val: string) {
    setSignatories(prev => prev.map((s, i) => i === idx ? { ...s, [field]: val } : s));
  }
  function removeSignatory(idx: number) {
    setSignatories(prev => prev.filter((_, i) => i !== idx));
  }
  function addSignatory() {
    setSignatories(prev => [...prev, { name: "", designation: "", stamp_area_label: "Seal" }]);
  }
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-5 py-4 border-b border-border">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-sm font-semibold text-foreground">Admission Offer Letter</CardTitle>
                  <p className="text-xs text-muted-foreground mt-0.5">Preview updates as you fill in the form.</p>
                </div>
            ...
            <CardContent className="p-5 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      {logoFileName && (
                        <span className="text-[10px] text-muted-foreground truncate flex-1">{logoFileName}</span>
                      )}
                    </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Field label="Valid until (AD)">
                    <Input className="h-8 text-xs" placeholder="e.g. 2024-12-31" value={validUntilAD} onChange={e => setValidUntilAD(e.target.value)} />
                  </Field>
                  <Field label="Valid until (BS)">
                    <Input className="h-8 text-xs" placeholder="e.g. 2081 Poush 15" value={validUntilBS} onChange={e => setValidUntilBS(e.target.value)} />
                  </Field>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Field label="Citizenship no.">
                    <Input className="h-8 text-xs" placeholder="e.g. 43-01-76-05231" value={citizenshipNo} onChange={e => setCitizenshipNo(e.target.value)} />
                  </Field>
                  <Field label="Date of birth (AD)">
                    <Input className="h-8 text-xs" placeholder="e.g. 2001-05-12" value={studentDobAD} onChange={e => setStudentDobAD(e.target.value)} />
                  </Field>
                  <Field label="Date of birth (BS)">
                    <Input className="h-8 text-xs" placeholder="e.g. 2058 Jestha 29" value={studentDobBS} onChange={e => setStudentDobBS(e.target.value)} />
                  </Field>
                  <Field label="Father's name">
                    <Input className="h-8 text-xs" placeholder="Father's full name" value={fatherName} onChange={e => setFatherName(e.target.value)} />
                  </Field>
                  <Field label="Mother's name">
                    <Input className="h-8 text-xs" placeholder="Mother's full name" value={motherName} onChange={e => setMotherName(e.target.value)} />
                  </Field>
                  <Field label="Permanent address">
                    <Input className="h-8 text-xs" placeholder="Ward / VDC / Municipality" value={permanentAddress} onChange={e => setPermanentAddress(e.target.value)} />
                  </Field>
                  <Field label="District">
                    <Input className="h-8 text-xs" placeholder="e.g. Kathmandu" value={district} onChange={e => setDistrict(e.target.value)} />
                  </Field>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label="Program short name *">
                    <Input className="h-8 text-xs" placeholder="e.g. BBA" value={progName} onChange={e => setProgName(e.target.value)} />
                  </Field>
                  <Field label="Program full name">
                    <Input className="h-8 text-xs" placeholder="e.g. Bachelor of Business Administration" value={progFullName} onChange={e => setProgFullName(e.target.value)} />
                  </Field>
                  <Field label="Program affiliation">
                    <Input className="h-8 text-xs" placeholder="e.g. Pokhara University" value={progAffiliation} onChange={e => setProgAffiliation(e.target.value)} />
                  </Field>
                  <Field label="Duration (years)">
                    <Input className="h-8 text-xs" type="number" placeholder="e.g. 4" value={durationYears} onChange={e => setDurationYears(e.target.value)} />
                  </Field>
                  <Field label="Total semesters">
                    <Input className="h-8 text-xs" type="number" placeholder="e.g. 8" value={totalSemesters} onChange={e => setTotalSemesters(e.target.value)} />
                  </Field>
                  <Field label="Credit hours">
                    <Input className="h-8 text-xs" type="number" placeholder="e.g. 120" value={creditHours} onChange={e => setCreditHours(e.target.value)} />
                  </Field>
                  ...
                  <Field label="Intake month (BS)">
                    <Input className="h-8 text-xs" placeholder="e.g. Bhadra 2081" value={intakeMonthBS} onChange={e => setIntakeMonthBS(e.target.value)} />
                  </Field>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label="Admission fee (one-time)">
                    <Input className="h-8 text-xs" type="number" placeholder="0" value={admissionFee} onChange={e => setAdmissionFee(e.target.value === "" ? "" : Number(e.target.value))} />
                  </Field>
                  <Field label="Tuition fee / semester">
                    <Input className="h-8 text-xs" type="number" placeholder="0" value={tuitionPerSem} onChange={e => setTuitionPerSem(e.target.value === "" ? "" : Number(e.target.value))} />
                  </Field>
                  <Field label="Exam fee / semester">
                    <Input className="h-8 text-xs" type="number" placeholder="0" value={examFeePerSem} onChange={e => setExamFeePerSem(e.target.value === "" ? "" : Number(e.target.value))} />
                  </Field>
                  <Field label="Lab fee / semester">
                    <Input className="h-8 text-xs" type="number" placeholder="0" value={labFeePerSem} onChange={e => setLabFeePerSem(e.target.value === "" ? "" : Number(e.target.value))} />
                  </Field>
                  <div className="sm:col-span-2">
                    <Field label={`Total (approx) — auto: ${autoTotal > 0 ? fmtNPR(autoTotal) : "fill fields above"}`}>
                      <div className="flex gap-2 items-center">
                        <Input
                          className={`h-8 text-xs flex-1 ${!totalManual ? "bg-muted/40 text-[oklch(0.42_0.18_145)] font-semibold" : ""}`}
                          type="number"
                          placeholder="Auto-calculated"
                          value={totalManual ? totalApprox : autoTotal > 0 ? autoTotal : ""}
                          readOnly={!totalManual}
                          onChange={e => { if (totalManual) setTotalApprox(e.target.value === "" ? "" : Number(e.target.value)); }}
                        />
                        <Button
                          type="button"
                          variant="outline"
                          ...
                          onClick={() => { setTotalManual(m => !m); setTotalApprox(""); }}
                        >
              ...
              <div>
                ...
                <div className="space-y-3">
                  {signatories.map((s, i) => (
                    <div key={i} className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-3 rounded-lg border border-border bg-muted/20">
                      <div className="space-y-1">
                        <Label className="text-[10px] text-muted-foreground">Name</Label>
                        <Input className="h-8 text-xs" placeholder="Signatory name" value={s.name} onChange={e => updateSignatory(i, "name", e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] text-muted-foreground">Designation</Label>
                        <Input className="h-8 text-xs" placeholder="e.g. Principal" value={s.designation} onChange={e => updateSignatory(i, "designation", e.target.value)} />
                      </div>
                      <div className="flex gap-2 items-end">
                        <div className="space-y-1 flex-1">
                          <Label className="text-[10px] text-muted-foreground">Stamp label</Label>
                          <Input className="h-8 text-xs" placeholder="e.g. Office Seal" value={s.stamp_area_label} onChange={e => updateSignatory(i, "stamp_area_label", e.target.value)} />
                        </div>
                        {signatories.length > 1 && (
                          <button type="button" aria-label="Remove signatory" onClick={() => removeSignatory(i)} className="mb-0.5 text-muted-foreground hover:text-destructive shrink-0">
                            <X className="w-3.5 h-3.5" />
                          </button>
                        )}
                      </div>
                  ...
                  <Button type="button" variant="outline" size="sm" className="h-7 text-xs gap-1.5" onClick={addSignatory}>
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-4 py-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] text-muted-foreground">NRB-compliant offer letter</span>
              </div>
            </CardHeader>
            <CardContent className="p-3">
              <div className="bg-white rounded-lg p-4 font-mono text-[9.5px] border border-border shadow-sm leading-relaxed">
                ...
                <div className="bg-primary/10 text-center py-1.5 mb-3 rounded">
                  <p className="text-[9.5px] font-bold text-primary">LETTER OF ADMISSION OFFER</p>
                  <p className="text-[8px] text-primary/80">
                    {progFullName || progName ? <><L v={progFullName || progName} p="Program" /></> : <span className="italic text-muted-foreground/40">[Program]</span>}
                    {(progAffiliation || collegeAffiliation) && <> · {progAffiliation || collegeAffiliation}</>}
                  </p>
                </div>
                ...
                <div className="bg-muted/40 rounded p-2 mb-2 text-[8.5px] leading-snug">
                  <strong>To,</strong><br />
                  <strong><L v={studentName} p="Student name" /></strong>
                  {fatherName && <> &nbsp;| S/O: {fatherName}</>}
                  {motherName && <> / D/O: {motherName}</>}<br />
                  Citizenship No.: <L v={citizenshipNo} p="—" />
                  &nbsp; DOB: {studentDobAD || <span className="text-muted-foreground/40 italic">[DOB]</span>}
                  {studentDobBS && ` (${studentDobBS} BS)`}<br />
                ...
                <p className="text-[8.5px] text-muted-foreground leading-snug mb-2">
                  Dear <strong className="text-foreground"><L v={studentName} p="Student" /></strong>, we are pleased to inform you that{" "}
                  <strong className="text-foreground"><L v={collegeName} p="College" /></strong> has{" "}
                  <strong className="text-[oklch(0.42_0.18_145)]">selected you for admission</strong> to{" "}
                  <L v={progFullName || progName} p="the program" /> for AY {academicYearBS}.
                  {(validUntilAD || validUntilBS) && ` Valid until ${validUntilAD || ""}${validUntilBS ? ` (${validUntilBS} BS)` : ""}.`}
                </p>

                {/* Program table */}
                <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">Program Details</p>
                <table className="w-full text-[8px] border-collapse mb-2">
                  <tbody>
                    {[
                      ["Program",      (progFullName || progName) || "—"],
                      ["Affiliation",  progAffiliation || collegeAffiliation || "—"],
                      ["Duration",     durationYears ? `${durationYears} yr${Number(durationYears)!==1?"s":""}` : "—"],
                      ...
                      <tr key={f} className={i % 2 === 1 ? "bg-muted/30" : ""}>
                        <td className="px-1.5 py-0.5 border border-border/50 text-muted-foreground w-[45%]">{f}</td>
                ...
                {/* Fee table */}
                <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">Fee Structure (NPR)</p>
                <table className="w-full text-[8px] border-collapse mb-2">
                  <tbody>
                    {[
                      ...
                      ["Total (approx)",     displayTotal > 0 ? fmtNPR(displayTotal) + "/-" : "—"],
                    ].map(([f, v], i) => (
                      <tr key={f} className={i % 2 === 1 ? "bg-muted/30" : ""}>
                        <td className="px-1.5 py-0.5 border border-border/50 text-muted-foreground w-[45%]">{f}</td>
                        <td className={`px-1.5 py-0.5 border border-border/50 ${f === "Total (approx)" ? "font-bold text-[oklch(0.42_0.18_145)]" : "font-medium"}`}>{v}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {conditions.some(Boolean) && (
                  <>
                    <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">Conditions</p>
                    <ol className="list-decimal list-inside space-y-0.5 mb-2">
                      {conditions.filter(Boolean).map((c, i) => (
                        <li key={i} className="text-[8px] text-muted-foreground leading-snug">{c}</li>
                      ))}
                    </ol>
                  </>
                )}
                ...
                <div className="bg-[oklch(0.62_0.18_145)]/8 border-l-2 border-[oklch(0.62_0.18_145)] px-2 py-1.5 mb-2 rounded-r text-[8px] leading-snug">
                  ...
                  <L v={collegeName} p="College" /> certifies this offer for bank education loan processing.
                  Total fee: <strong>{displayTotal > 0 ? fmtNPR(displayTotal) + "/-" : "[total]"}</strong> (NPR).<br />
                ...
                <div className="flex justify-between mt-3 pt-2 border-t border-border">
                  {signatories.map((s, i) => (
                    <div key={i} className="flex-1 text-center text-[7.5px]">
                      ...
                      <p className="font-bold text-foreground text-[8px]">{s.name || s.designation}</p>
                      <p className="text-muted-foreground">{s.designation}</p>
                      <p className="text-muted-foreground/50 italic">[{s.stamp_area_label}]</p>
                    </div>
                  ))}
                </div>
