...
const AGREEMENT_TYPES = [
  ...
  { icon: Zap,        label: "College–GenZ Loan MOU",    key: "genz"        as const, count: 1  },
  { icon: Users,      label: "Student loan agreement",   key: "student"     as const, count: 28 },
  { icon: UserCheck,  label: "Guarantor undertaking",    key: "guarantor"   as const, count: 28 },
  { icon: ArrowRight, label: "Disbursement instruction", key: "disbursement"as const, count: 18 },
] as const;
...
const MOU_STATUS = [
  ...
  { name: "GenZ Loan",      badge: "Active 2083",  type: "digital" as const },
...
const mouBadgeStyle = {
  ...
  digital: "bg-[oklch(0.528_0.113_235.573)]/10 text-[oklch(0.528_0.113_235.573)] border-0",
...
function CertToggle({ label, checked, onChange, positiveLabel = "Yes", negativeLabel = "No" }: {
...
export default function AgreementsView() {
  ...
  // ── Document (shared) ──────────────────────────────────────────────────────
  const [refNo,        setRefNo]        = useState(() => `AGR-${new Date().getFullYear()}-${String(Date.now()).slice(-4)}`);
  ...
  // ── QR ─────────────────────────────────────────────────────────────────────
  const [qrToken,     setQrToken]     = useState(() => `AGR-${String(Date.now()).slice(-8)}`);
  const [qrVerifyUrl, setQrVerifyUrl] = useState(() => `verify.genzloan.com.np/doc/AGR-${String(Date.now()).slice(-8)}`);
  ...
  const typeLabel  = AGREEMENT_TYPES.find(a => a.key === activeType)?.label ?? "Agreement";
  ...
  const handleDownloadMOU = () => {
    if (!collegeName) { toast.error("Fill in college name."); return; }
    const partyBName = activeType === "genz" ? "GenZ Loan Pvt. Ltd." : bank;
    const clauses = [
      `${collegeName} shall verify student enrollment and issue certified documents via the GenZ Loan platform, acting as facilitating intermediary only.`,
      ...
      `GenZ Loan Pvt. Ltd. shall provide the digital platform for document generation, QR verification, and application dispatch, charging 0.25% origination fee per disbursed loan.`,
      `Loan disbursements shall be made directly to ${collegeName}'s designated fee account at ${partyBName} in semester tranches.`,
      `${collegeName} shall notify ${partyBName} and GenZ Loan of any student dropout or suspension within 15 days.`,
    ...
    ].map((c, i) => `<div style="display:flex;gap:10px;margin-bottom:9px"><span style="font-weight:700;min-width:16px">${i + 1}.</span><p style="margin:0;font-size:9.5pt;color:#374151;line-height:1.65">${c}</p></div>`).join("");
...
<div class="pc">+ GenZ Loan Pvt. Ltd. (Party C — Digital Platform Intermediary)</div>
...
<div class="sigs">
  ...
  <div class="sig"><div class="sl"></div><div class="sn">CEO / Authorised Rep.</div><div class="ss">GenZ Loan Pvt. Ltd.</div><div class="ss">Party C</div></div>
</div>
<div class="ft">Generated via GenZ Loan Digital Platform · NRB Unified Directive 2081</div>
<script>window.onload=function(){setTimeout(function(){window.print();window.onfocus=function(){window.close();}},200);}</script>
  ...
  const handleDownloadStudentAgreement = () => {
    if (!collegeName || !studentName) { toast.error("Fill in college name and student name."); return; }
    const certRows = [
      ["Currently Enrolled",  isEnrolled      ? "✓ Yes — Confirmed"        : "✗ Not confirmed"],
      ["Academic Backlogs",   hasBacklogs     ? "✗ Yes (has backlogs)"      : "✓ None"],
      ["Disciplinary Hold",   disciplinaryHold? "✗ Yes"                     : "✓ None"],
      ["Fee Dues (NPR)",      feeDue > 0      ? `Rs ${feeDue.toLocaleString("en-IN")}` : "✓ Nil"],
    ].map(([f, v], i) => `<tr style="background:${i % 2 === 1 ? "#f9fafb" : "#fff"}"><td style="padding:6px 10px;border:1px solid #d1d5db;width:45%;color:#6b7280">${f}</td><td style="padding:6px 10px;border:1px solid #d1d5db;font-weight:500">${v}</td></tr>`).join("");
...
${[["Student Name", studentName], ["TU / Univ. Roll No.", tuRollNo || "—"], ["Enrollment No.", enrollmentNo || "—"], ["Program / Course", programName || "—"], ["Year / Semester", `${currentYear} Year · Sem ${currentSem}`], ["Academic Year (BS)", academicYearBS], ["Enrollment Status", studentStatus]].map(([f, v], i) => `<tr style="background:${i % 2 === 1 ? "#f9fafb" : "#fff"}"><td style="padding:6px 10px;border:1px solid #d1d5db;color:#6b7280">${f}</td><td style="padding:6px 10px;border:1px solid #d1d5db;font-weight:500">${v}</td></tr>`).join("")}
</tbody></table>
...
<script>window.onload=function(){setTimeout(function(){window.print();window.onfocus=function(){window.close();}},200);}</script>
  ...
  const handleDownload = () => isMOU ? handleDownloadMOU() : handleDownloadStudentAgreement();

  const handleGenerate = () => {
    if (!collegeName) { toast.error("Fill in college name first."); return; }
    if (isStudent && !studentName) { toast.error("Fill in student name."); return; }
    setGenerated(true);
    toast.success("Agreement ready", { description: "Click Download PDF to save." });
  };
    ...
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Select an agreement type — the form and live preview update accordingly.
        </p>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.08 }}
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-4 py-3 border-b border-border">
              <CardTitle className="text-xs font-semibold text-foreground">Agreement types</CardTitle>
            </CardHeader>
            <CardContent className="p-1.5 space-y-0.5">
              {AGREEMENT_TYPES.map((t) => (
                <button type="button" key={t.key}
                  onClick={() => { setActiveType(t.key); setGenerated(false); }}
                  className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-left transition-colors ${activeType === t.key ? "bg-primary/10" : "hover:bg-muted"}`}>
                  <div className="flex items-center gap-2 min-w-0">
                    <t.icon className={`w-3.5 h-3.5 shrink-0 ${activeType === t.key ? "text-primary" : "text-muted-foreground"}`} />
                    <span className={`text-[11px] font-medium truncate ${activeType === t.key ? "text-primary" : "text-foreground"}`}>{t.label}</span>
                  </div>
                  <Badge variant="outline" className="text-[9px] shrink-0 ml-1.5 px-1.5">{t.count}</Badge>
                </button>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-4 py-3 border-b border-border">
              <CardTitle className="text-xs font-semibold text-foreground">MOU status</CardTitle>
            </CardHeader>
            <CardContent className="px-4 py-3 space-y-2">
              {MOU_STATUS.map((m) => (
                <div key={m.name} className="flex items-center justify-between">
                  <span className="text-[11px] text-foreground">{m.name}</span>
                  <Badge className={`text-[9px] font-semibold ${mouBadgeStyle[m.type]}`}>{m.badge}</Badge>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="border-border shadow-none">
            <CardHeader className="px-5 py-4 border-b border-border">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-sm font-semibold text-foreground">{typeLabel}</CardTitle>
                  <p className="text-xs text-muted-foreground mt-0.5">Preview updates live as you fill the form.</p>
                </div>
            ...
            <CardContent className="p-5 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    <Input className="h-8 text-xs" placeholder="https://..." value={logoFileName ? "" : logoUrl} disabled={!!logoFileName} onChange={e => setLogoUrl(e.target.value)} />
                    {logoUrl && (
                      <div className="flex items-center gap-2 p-2 rounded-lg border border-border bg-muted/20">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={logoUrl} alt="Logo" className="h-10 w-10 object-contain rounded bg-white p-0.5 border border-border shrink-0" onError={e => (e.currentTarget.style.display = "none")} />
                        <p className="text-[10px] text-foreground flex-1 truncate">{logoFileName || "Logo from URL"}</p>
              ...
              <div>
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border">Document</p>
              ...
              {isMOU && (
                <>
                  <div>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border">
                      {activeType === "genz" ? "GenZ Loan (Party B)" : "Bank / Lender (Party B)"}
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {activeType === "mou" ? (
                        <Field label="Bank">
                          <Select value={bank} onValueChange={setBank}>
                            ...
                            <SelectContent>
                              {["NIC Asia Bank Ltd.", "NMB Bank Ltd.", "Laxmi Sunrise Bank Ltd.", "Kumari Bank Ltd.", "Global IME Bank Ltd."].map(b => (
                        ...
                        <Field label="Party B">
                          <Input className="h-8 text-xs bg-muted/40" value="GenZ Loan Pvt. Ltd." readOnly />
                        </Field>
                      )}
                      <Field label="Branch manager / Signatory">
                        <Input className="h-8 text-xs" placeholder="Full name" value={branchManager} onChange={e => setBranchManager(e.target.value)} />
                      </Field>
                      <Field label="College authorized signatory">
                        <Input className="h-8 text-xs" placeholder="Principal / Campus Chief" value={principalName} onChange={e => setPrincipalName(e.target.value)} />
                      </Field>
                      <Field label="Execution date (BS)">
                        <Input className="h-8 text-xs" placeholder="e.g. 2081 Ashadh 15" value={executionDate} onChange={e => setExecutionDate(e.target.value)} />
                      </Field>
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border">Loan Terms</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <Field label="Max loan per student (NPR)">
                        <Input className="h-8 text-xs" placeholder="e.g. Rs 10,00,000" value={maxLoan} onChange={e => setMaxLoan(e.target.value)} />
                      </Field>
                      <Field label="Interest rate">
                        <Input className="h-8 text-xs" placeholder="e.g. 9.5% p.a. floating" value={rate} onChange={e => setRate(e.target.value)} />
                      </Field>
                      <Field label="Moratorium period">
                        <Select value={moratorium} onValueChange={setMoratorium}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="course+1"  className="text-xs">Course duration + 1 year</SelectItem>
                            <SelectItem value="course+6m" className="text-xs">Course duration + 6 months</SelectItem>
                          </SelectContent>
                        </Select>
                      </Field>
                      <Field label="MOU validity (years)">
                        <Select value={validYears} onValueChange={setValidYears}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="1" className="text-xs">1 year</SelectItem>
                            <SelectItem value="2" className="text-xs">2 years</SelectItem>
                            <SelectItem value="3" className="text-xs">3 years</SelectItem>
                          </SelectContent>
                        </Select>
                      </Field>
                    </div>
              ...
              {isStudent && (
                <>
                  <div>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border">Student</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <Field label="Current year">
                        <Select value={currentYear} onValueChange={setCurrentYear}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>{YEARS.map(y => <SelectItem key={y} value={y} className="text-xs">{y} Year</SelectItem>)}</SelectContent>
                        </Select>
                      </Field>
                      <Field label="Current semester">
                        <Select value={currentSem} onValueChange={setCurrentSem}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>{SEMS.map(s => <SelectItem key={s} value={s} className="text-xs">Semester {s}</SelectItem>)}</SelectContent>
                        </Select>
                      </Field>
                      <Field label="Academic year (BS)">
                        <Select value={academicYearBS} onValueChange={setAcademicYearBS}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {["2080-081","2081-082","2082-083","2083-084"].map(y => <SelectItem key={y} value={y} className="text-xs">{y}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </Field>
                      <Field label="Status">
                        <Select value={studentStatus} onValueChange={setStudentStatus}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {["Active","On Leave","Suspended"].map(s => <SelectItem key={s} value={s} className="text-xs">{s}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </Field>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="rounded-lg border border-border overflow-hidden mb-3">
                      <CertToggle label="Student is currently enrolled"  checked={isEnrolled}       onChange={setIsEnrolled} />
                      <CertToggle label="Has academic backlogs"           checked={hasBacklogs}      onChange={setHasBacklogs} />
                      <CertToggle label="Disciplinary hold on record"     checked={disciplinaryHold} onChange={setDisciplinaryHold} />
                    </div>
                    <Field label="Pending fee dues (NPR) — 0 = no dues">
                      <Input className="h-8 text-xs" type="number" min={0} placeholder="0"
                        value={feeDueRs} onChange={e => setFeeDueRs(e.target.value === "" ? "" : Number(e.target.value))} />
                    </Field>
                  </div>
                  <Separator />
                </>
              ...
              {/* QR — always visible */}
              <div>
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border">QR / Verification</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label="Token">
              ...
              {generated && (
                <div className="flex items-center gap-2 px-4 py-3 bg-[oklch(0.62_0.18_145)]/10 rounded-xl border border-[oklch(0.62_0.18_145)]/20">
                  ...
                  <p className="text-xs text-[oklch(0.42_0.18_145)] font-medium">
                    Agreement ready.{" "}
                    <button type="button" className="underline" onClick={handleDownload}>Download PDF</button>
                  </p>
                </div>
              )}
            ...
            <CardContent className="p-3">
              <div className="bg-white rounded-lg p-4 font-mono text-[9.5px] border border-border shadow-sm leading-relaxed">
                ...
                <div className="flex justify-between text-[8px] text-muted-foreground mb-2">
                  ...
                  <span><strong>Date:</strong> {displayDate}{issuedDateBS && ` / ${issuedDateBS} BS`}</span>
                </div>

                <div className="bg-primary/10 text-center py-1.5 mb-3 rounded">
                  <p className="text-[9.5px] font-bold text-primary">{typeLabel.toUpperCase()}</p>
                  {isMOU   && <p className="text-[8px] text-primary/70">NRB Compliant · Education Loan Facilitation</p>}
                  {isStudent && <p className="text-[8px] text-primary/70"><L v={programName} p="Program" /> · <L v={collegeName} p="College" /></p>}
                </div>
                  ...
                  <>
                    <div className="grid grid-cols-[1fr_28px_1fr] gap-2 items-center mb-2">
                      <div className="border border-border rounded-lg p-2">
                        <p className="text-[9px] font-bold"><L v={collegeName} p="College" /></p>
                        <p className="text-[8px] text-muted-foreground">Party A</p>
                      </div>
                      <p className="text-center text-[8px] font-bold text-muted-foreground">AND</p>
                      <div className="border border-border rounded-lg p-2">
                        <p className="text-[9px] font-bold">{activeType === "genz" ? "GenZ Loan Pvt. Ltd." : bank}</p>
                        <p className="text-[8px] text-muted-foreground">Party B</p>
                      </div>
                    </div>
                    <p className="text-center text-[8px] text-[oklch(0.528_0.113_235.573)] font-semibold mb-2">+ GenZ Loan Pvt. Ltd. (Party C)</p>
                    <table className="w-full text-[8px] border-collapse mb-2">
                      <tbody>
                        {[["Max Loan",maxLoan||"—"],["Rate",rate||"—"],["Moratorium",MORATORIUM_LABELS[moratorium]??moratorium],["Validity",`${validYears} yr${validYears!=="1"?"s":""}`]].map(([k,v],i)=>(
                          <tr key={k} className={i%2===1?"bg-muted/40":""}>
                            <td className="px-1.5 py-0.5 border border-border/50 text-muted-foreground w-[40%]">{k}</td>
                            <td className="px-1.5 py-0.5 border border-border/50 font-semibold">{v}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <div className="bg-[oklch(0.62_0.18_145)]/8 border-l-2 border-[oklch(0.62_0.18_145)] px-2 py-1.5 mb-2 rounded-r text-[8px]">
                      <span className="text-primary">QR: {qrVerifyUrl} · {qrToken}</span>
                    </div>
                    <div className="flex justify-between mt-3 pt-2 border-t border-border">
                      {[{n:principalName||"Principal",o:collegeName||"College",p:"Party A"},{n:branchManager||"Manager",o:activeType==="genz"?"GenZ Loan":bank,p:"Party B"},{n:"CEO",o:"GenZ Loan",p:"Party C"}].map(s=>(
                        <div key={s.p} className="flex-1 text-center text-[7.5px]">
                ...
                {isStudent && (
                  <>
                    <p className="text-[8.5px] text-muted-foreground leading-snug mb-2">
                      This certifies that <strong className="text-foreground"><L v={studentName} p="Student name" /></strong> is enrolled in{" "}
                      <strong className="text-foreground"><L v={programName} p="Program" /></strong> at{" "}
                      <strong className="text-foreground"><L v={collegeName} p="College" /></strong> — {currentYear} Year / Sem {currentSem}, AY {academicYearBS}. Status: <strong className="text-foreground">{studentStatus}</strong>.
                    </p>
                    <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">Student Details</p>
                    <table className="w-full text-[8px] border-collapse mb-2">
                      <tbody>
                        {[["Name",studentName||"—"],["TU Roll",tuRollNo||"—"],["Enroll. No",enrollmentNo||"—"],["Program",programName||"—"],["Yr / Sem",`${currentYear} · Sem ${currentSem}`],["AY (BS)",academicYearBS],["Status",studentStatus]].map(([f,v],i)=>(
                          <tr key={f} className={i%2===1?"bg-muted/30":""}>
                            <td className="px-1.5 py-0.5 border border-border/50 text-muted-foreground w-[38%]">{f}</td>
                    ...
                    <p className="text-[8px] font-bold text-foreground mb-1 border-b border-border pb-0.5">
                      <ShieldCheck className="w-3 h-3 inline mr-0.5 text-[oklch(0.42_0.18_145)]" /> Certifications
                    </p>
                    <table className="w-full text-[8px] border-collapse mb-2">
                      <tbody>
                        {[
                          {label:"Enrolled",         v:certIcon(isEnrolled,true),       t:isEnrolled?"Yes":"No"},
                          {label:"Backlogs",          v:certIcon(hasBacklogs,false),      t:hasBacklogs?"Yes":"None"},
                          {label:"Disciplinary Hold", v:certIcon(disciplinaryHold,false), t:disciplinaryHold?"Yes":"None"},
                          {label:"Fee Dues",          v:certIcon(feeDue===0,true),        t:feeDue>0?`Rs ${feeDue.toLocaleString("en-IN")}`:"Nil"},
                        ].map(({label,v,t},i)=>(
                          <tr key={label} className={i%2===1?"bg-muted/30":""}>
                            <td className="px-1.5 py-0.5 border border-border/50 text-muted-foreground w-[40%]">{label}</td>
                            <td className="px-1.5 py-0.5 border border-border/50">{v}{t}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <div className="bg-[oklch(0.62_0.18_145)]/8 border-l-2 border-[oklch(0.62_0.18_145)] px-2 py-1.5 mb-2 rounded-r text-[8px]">
                      <strong>Verified:</strong> <L v={collegeName} p="College" /><br />
                      <span className="text-primary">QR: {qrVerifyUrl} · {qrToken}</span>
                    </div>
                    <div className="flex justify-between mt-3 pt-2 border-t border-border">
                      {["Registrar","Principal [Stamp]","Student"].map(sig=>(
                        <div key={sig} className="flex-1 text-center text-[7.5px]">
                          <div className="border-t border-muted-foreground/30 mt-5 mb-1" />
                          <p className="font-bold text-foreground text-[8px]">{sig}</p>
                          <p className="text-muted-foreground">{sig==="Student"?studentName||"Student":collegeName||"College"}</p>
                        </div>
                      ))}
                    </div>
                  </>
