...
import { useSupporterApplications } from "./hooks/useSupporterApplications";
...
/** "My Queue" tab — applications cleared by the Initiator and awaiting (or sent back
 *  for) this Supporter's own Support/Send Back action. Reuses the same hook that
 *  backs the Support review entry point. */
function MyQueueTable({ search }: { search: string }) {
  const router = useRouter();
  const basePath = useDashboardBasePath();
  const debouncedSearch = useDebounce(search, 300);
  const { data, isLoading } = useSupporterApplications();
  ...
  if (rows.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p className="text-xs text-muted-foreground">Try adjusting your search, or check back once the Initiator clears more applications.</p>
      </div>
    );
  }
...
/**
 * Supporter's "Applications" page — mirrors the Initiator's Applications page
 * structure exactly (tabs, search, same "All" table). No "New Application" button:
 * only the Initiator creates applications. "My Queue" shows this Supporter's own
 * actionable items instead of the Initiator's college-verified intake queue.
 */
