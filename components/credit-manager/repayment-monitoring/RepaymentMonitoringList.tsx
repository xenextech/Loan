...
import { Search, Inbox, ChevronLeft, ChevronRight, Activity } from "lucide-react";
...
/** Credit Manager's Repayment Monitoring entry point — lists the approved
 *  portfolio (same `disbursement` filter as EMI Schedule, since only loans
 *  that reached disbursement can be monitored for repayment) and hands off
 *  to `/repayment-monitoring/[id]`, which renders the per-application
 *  monitoring snapshot from GET /dashboard/repayment/{applicationId}/status —
 *  distinct from the raw amortization table under EMI Schedule. */
export function RepaymentMonitoringList() {
  ...
  return (
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Repayment Monitoring</h1>
        <p className="text-sm text-muted-foreground mt-1">Select a loan to view its repayment health — status, aging, and outstanding balance.</p>
      </motion.div>
                  ...
                  <TableBody>
                    {rows.map((app) => (
                      <TableRow
                        ...
                        onClick={() => router.push(`${basePath}/repayment-monitoring/${app.id}`)}
                      >
                        ...
                        <TableCell className="py-3.5 text-right pr-5">
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`${basePath}/repayment-monitoring/${app.id}`);
                            }}
                          >
