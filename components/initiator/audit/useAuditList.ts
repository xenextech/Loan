...
/**
 * Real applications, presented as the audit-ledger queue — backed by the same
 * GET /applications/initiator/college-verified endpoint the other initiator modules use.
 * There's no backend endpoint yet for immutable action logging, so every row is honestly
 * a single "Awaiting Review" stage until that data exists.
 */
