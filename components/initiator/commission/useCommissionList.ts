...
/**
 * Real applications, presented as the commission queue — backed by the same
 * GET /applications/initiator/college-verified endpoint the other initiator modules use.
 * There's no backend endpoint yet for commission tracking, so every row is honestly a
 * single "Awaiting Review" stage until that data exists.
 */
export function useCommissionList(): {
  data: CommissionQueueItem[];
  isLoading: boolean;
} {
