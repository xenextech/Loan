...
export function NotificationList() {
  ...
  const { data: applications, isLoading } = useNotifications();
  ...
  return (
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">Per-applicant notification history and delivery status.</p>
      </motion.div>
                ...
                <TableBody>
                  {filtered.map((app) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/initiator/notification/${app.id}`)}
                    >
                      ...
                      <TableCell className="py-3.5 text-right pr-5">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/initiator/notification/${app.id}`);
                          }}
                        >
                          View
                        </Button>
