...
import { useEmiSchedules } from "./useEmiSchedules";
...
export function EmiScheduleList() {
  ...
  const { data: applications, isLoading } = useEmiSchedules();
  ...
  return (
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">EMI Schedule</h1>
        <p className="text-sm text-muted-foreground mt-1">Repayment schedules and overdue tracking, per applicant.</p>
      </motion.div>
                ...
                <TableBody>
                  {filtered.map((app) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/initiator/emi-schedule/${app.id}`)}
                    >
                      ...
                      <TableCell className="py-3.5 text-right pr-5">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/initiator/emi-schedule/${app.id}`);
                          }}
                        >
                          View
                        </Button>
