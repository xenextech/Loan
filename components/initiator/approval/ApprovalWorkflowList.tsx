...
export function ApprovalWorkflowList() {
  ...
  const colleges = useMemo(
    () => Array.from(new Set(applications.map((app) => app.branch))).sort(),
    [applications],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return applications.filter((app) => {
      ...
      const matchesCollege = collegeFilter === "all" || app.branch === collegeFilter;
    ...
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Applications moving through Initiator → Supporter → Checker → Approver review.
        </p>
      </motion.div>
