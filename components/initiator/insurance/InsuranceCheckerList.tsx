...
import { useInsuranceList } from "./useInsuranceList";
...
export function InsuranceCheckerList() {
  ...
  const { data: applications, isLoading } = useInsuranceList();
  ...
  return (
    <div className="p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Insurance Checker</h1>
        <p className="text-sm text-muted-foreground mt-1">Policy attachment and expiry tracking, per applicant.</p>
      </motion.div>
                ...
                <TableBody>
                  {filtered.map((app) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/initiator/insurance-checker/${app.id}`)}
                    >
                      ...
                      <TableCell className="py-3.5 text-right pr-5">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/initiator/insurance-checker/${app.id}`);
                          }}
                        >
                          View
                        </Button>
