...
import { useApproverApplications } from "./hooks/useApproverApplications";
...
/** "My Queue" tab — applications checked by the Checker and awaiting this
 *  Approver's own Approve/Reject/Send Back action. Reuses the same hook that
 *  backs the Approve review entry point. */
function MyQueueTable({ search }: { search: string }) {
  const router = useRouter();
  const basePath = useDashboardBasePath();
  const debouncedSearch = useDebounce(search, 300);
  const { data, isLoading } = useApproverApplications();
  ...
  if (rows.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p className="text-xs text-muted-foreground">Try adjusting your search, or check back once the Checker clears more applications.</p>
      </div>
    );
  }
...
/**
 * Approver's "Applications" page — mirrors the Initiator/Supporter/Checker
 * Applications page structure exactly (tabs, search, same "All" table). No "New
 * Application" button: only the Initiator creates applications. "My Queue" shows
 * this Approver's own actionable items instead.
 */
export default function ApproverApplications() {
